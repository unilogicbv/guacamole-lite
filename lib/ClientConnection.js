...
class ClientConnection {
    ...
    close(error) {
        if (this.state == this.STATE_CLOSED) {
            if (this.server.clientOptions.log.level >= this.server.LOGLEVEL.NORMAL) {
                this.server.clientOptions.log.stdLog('Connection Already Closed');
            }
            return;
        }
        ...
        if (this.guacdClient) {
            if (this.server.clientOptions.log.level >= this.server.LOGLEVEL.NORMAL) {
                this.server.clientOptions.log.stdLog('Closing GaucD Connection....');
            }
            this.guacdClient.close();
        }

        if (this.server.clientOptions.log.level >= this.server.LOGLEVEL.NORMAL) {
            this.server.clientOptions.log.stdLog('Closing WebSocket...');
        }
        this.webSocket.removeAllListeners('close');
        this.webSocket.close();

        if (this.server.clientOptions.log.level >= this.server.LOGLEVEL.NORMAL) {
            this.server.clientOptions.log.stdLog('Deleting Active Connection...');
        }
    ...
    mergeConnectionOptions() {
        ...
        // check if connection type is in list
        if(this.server.clientOptions.allowedUnencryptedConnectionSettings[this.connectionType]) {
            Object
                .keys(this.query)
        ...
        DeepExtend(
            compiledSettings,
            // if no default setting for connection type, then return default
